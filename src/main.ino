...
// Set up limit switch objects for use by G28
...
// Interrupt Service Routines (ISRs) for the limit switches. These are called to
// detect faults
...
void loop() {
  ...
  if (manager.getLimitFault() && fsm.getState() != State::G28) {
    ...
    fsm.handleEvent(-1);  // Force FAULT state
  }
