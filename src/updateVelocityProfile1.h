 ...
 * --------------------------------------------------------------------
 *  resetVelocityProfile(); -> needed for each new movement command to reset the
 * internal state of the velocity profile pathVelocity = 0.0f;
 *  .
 *  .
 *  .
 *  .
 *  float updatedVelocity = updateVelocityProfile(dt, cruiseSpeed, acceleration,
 * pathVelocity, remainingDistance); analogWrite(velocityPin, updatedVelocity *
 * 255.0 / cruiseSpeed); // Scale to PWM range
 */

float updateVelocityProfile1(float dt, float cruiseSpeed, float acceleration,
                             float pathVelocity, float remainingDistance);
