...
void updateLeftEncoderCounts() {
  const bool a = digitalRead(pins::ENC_L_A);
  const bool b = digitalRead(pins::ENC_L_B);
  if (a == b) {
    ++leftEncoderCounts;
  } else {
    --leftEncoderCounts;
...
void updateRightEncoderCounts() {
  const bool a = digitalRead(pins::ENC_R_A);
  const bool b = digitalRead(pins::ENC_R_B);
  if (a == b) {
    ++rightEncoderCounts;
  } else {
    --rightEncoderCounts;
...
void isrLeftEncoder() { updateLeftEncoderCounts(); }
void isrRightEncoder() { updateRightEncoderCounts(); }
...
void setup() {
  ...
  pinMode(pins::ENC_L_A, INPUT_PULLUP);
  pinMode(pins::ENC_L_B, INPUT_PULLUP);
  pinMode(pins::ENC_R_A, INPUT_PULLUP);
  pinMode(pins::ENC_R_B, INPUT_PULLUP);
  ...
  attachInterrupt(digitalPinToInterrupt(pins::ENC_L_A), isrLeftEncoder, CHANGE);
  attachInterrupt(digitalPinToInterrupt(pins::ENC_R_A), isrRightEncoder,
                  CHANGE);
...
void loop() {
  ...
  if (state == FIRST_PASS) {
    driveMotor(pins::M1_DIR, pins::M1_PWM, 120);
    driveMotor(pins::M2_DIR, pins::M2_PWM, 120);
