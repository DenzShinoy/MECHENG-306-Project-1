...
void G1::beginMove(long target_x, long target_y) {
  ...
  getMaxSpeed(motorTarget_, currentPos, speedL_, speedR_);

  // Reinitialise PID controllers for the new move.
  pidL_ = PID(cfg::PID_KP, cfg::PID_KI, cfg::PID_KD, motorTarget_.a, speedL_);

  pidR_ = PID(cfg::PID_KP, cfg::PID_KI, cfg::PID_KD, motorTarget_.b, speedR_);
